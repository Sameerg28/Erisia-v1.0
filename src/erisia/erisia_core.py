...
def _generate_os_control_script_spec(user_input):
    ...
    script_code = str(data.get("script_code") or "").strip() or _fallback_os_script_from_request(user_input)
    if not script_code:
        return None

    skill_name = str(data.get("skill_name")).strip() or _derive_skill_name_from_request(user_input)
...
def _trim(text, max_chars=MAX_CONTEXT_CHARS):
...
def erisia_complete_brain(user_input, system_injection=None):
    ...
    mem = memory_system.query_memory(user_input, n_results=3)
    past_memory = _trim("\n".join(mem)) if mem else ""
    
    consciousness_data = _trim(identity_system.get_consciousness_context(), 1500)
    internal_state = _trim(identity_system.get_internal_state_summary(), 800)


    # --- DYNAMIC GRAPH RETRIEVAL ---
    graph_context = ""
    # We check if any known entities are mentioned in your prompt
    for node in list(graph_memory.graph.nodes())[:10]:
        if str(node).lower() in user_input.lower():
            graph_context += graph_memory.get_entity_context(node, depth=1) + "\n"
            
    if graph_context:
        graph_context = f"\nRELATIONAL MEMORY:\n{_trim(graph_context, 1000)}"

    goal_context = _trim(build_goal_context_text(limit=3), 1000)
    world_state_context = _trim(build_world_state_context_text(), 800)
    core_context_block = (
        f"CORE CONSCIOUSNESS:\n{consciousness_data}{goal_context}\n\n"
        f"DEEP MEMORY:\n{past_memory}{graph_context}\n\n"
        f"{world_state_context}\n\n"
        f"WEB DATA:\n{_trim(web_context, 800)}"
    )
    ...
    def build_messages(runtime_tools_snapshot):
        ...
        heuristic_rules = get_all_heuristics()[-5:]
        if heuristic_rules:
            heuristic_block = "[SYSTEM: GOLDEN HEURISTIC RULES]\n" + "\n".join(
                [f"- {rule}" for rule in heuristic_rules]
            )
            msg.append({"role": "system", "content": _trim(heuristic_block, 1500)})
        ...
        recent_memory = get_recent_context(limit=3)
        msg.append({
            "role": "system",
            "content": f"[SYSTEM: RECENT EPISODIC MEMORY]\n{_trim(recent_memory, 1000)}",
        })
    ...
    while True:
        ...
        try:
            ...
            if "400" in error_msg or "tool_use_failed" in error_msg:
                ...
                print("\n[CONTEXT TRIM]: Request too large — aggressively trimming context...")
                ...
                try:
                    ...
                    print("\n[CONTEXT TRIM]: Still too large — stripping all context, text-only fallback...")
                    messages = [
                        {"role": "system", "content": "You are Erisia. Respond concisely to Master Sameer."},
    ...
    if _voice_ready and erisia_reply and not erisia_reply.startswith("[SYSTEM") and not erisia_reply.startswith("[VOICE"):
        try:
            get_voice_manager().speak(erisia_reply)
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    consecutive_errors = 0
    _voice_ready = VOICE_AVAILABLE
    if _voice_ready:
        try:
            get_voice_manager()
            print("[Voice System: online - always listening mode]")
        except Exception as _vexc:
            _voice_ready = False
            print(f"[Voice System: unavailable - {_vexc}]")
    try:
        while True:
            if _voice_ready:
                print("\n🎤 Listening...")
            ...
            erisia_reply = erisia_complete_brain(user_input, system_injection=system_injection)
    finally:
        shutdown_event.set()
        if world_state_tracker:
